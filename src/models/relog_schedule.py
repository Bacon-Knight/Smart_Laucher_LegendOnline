...
class RelogScheduler:
    ...
    # Horários fixos de eventos no jogo (HH:MM)
    ...
    def calculate_next_relog_seconds(cls, default_interval_mins: int = 120) -> Tuple[int, str]:
        """
        Calcula quantos segundos faltam para o próximo relog automático.
        ...
        for t_str in cls.EVENT_TIMES:
            ...
            # Se o disparo de hoje já passou, calcula para o dia seguinte
            if pre_dt <= now:
                pre_dt += datetime.timedelta(days=1)
            pre_event_dts.append((pre_dt, t_str))

        # Encontra o disparo pré-evento mais próximo
        next_pre_event_dt, event_name = min(pre_event_dts, key=lambda x: x[0])
        secs_to_pre_event = int((next_pre_event_dt - now).total_seconds())
        ...
        if secs_to_pre_event < default_secs:
            return secs_to_pre_event, f"Relog preventivo (15 min antes do evento das {event_name})"
        else:
            return default_secs, f"Intervalo padrão de {default_interval_mins} minutos"
